...
from frc_rebuilt.calibrated_controls import ShooterSetpoint, CalibratedRobotControl, wrap_angle
from frc_rebuilt.robot_model import (
    ShooterInputs,
    ShooterState,
    ShooterStateMachine,
    quat_wxyz_to_matrix,
)
from frc_rebuilt.swerve import (
    MAX_WHEEL_SPEED_MPS,
    MODULE_ORDER,
    MODULE_POSITIONS,
    WHEEL_RADIUS_M,
...
# FRC's source prefab exposes eight real PRELOAD anchors.  There is deliberately
...
# the raw FRC sensor target sits ~0.17 m outboard of the funnel centre
...
# Measured hub structure bounds (assets/fresh_frc field_meshes): footprint
...
# balls follow this visible conveyor path at the FRC ballshooting_v2 speed
...
# in compact mode and blocked the 0.591 m FRC trench roof.
...
# 0.02 (CloseStorage).  The 22.25 in FRC/FIRST trench opening is 0.565 m high.
...
def _hopper_slots() -> np.ndarray:
    """The eight FRC preloads begin in the two four-wide turret rows."""
...
class CompetitionRobotController:
    ...
    def _sync_intake_transit(self, fuel_view: Any, dt_s: float) -> int:
        """Move captured balls through the FRC-style force-zone conveyor."""
