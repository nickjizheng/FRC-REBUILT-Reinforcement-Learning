...
policy. Files: `src/frc_rebuilt/rl/distributed.py` (transport, unit-tested),
...
ls -la /dev/shm/frc_dist/weights/                              # weights being republished
```
...
Note the transport dirs (`/dev/shm/frc_dist`) are RAM — they vanish on reboot
...
- `assets/fresh_frc/` - extracted field data (required at runtime)
...
- `docs/` - rules, architecture notes, audits, and operational runbooks
