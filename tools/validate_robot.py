...
Scenarios (single headless Isaac session, sequential):
  A. flat-ground top speed at full stick (FRC target ~3.5 m/s),
  B. turn-in-place yaw rate at full turn stick (FRC target ~100 deg/s),
  C. intake capture: drive over three staged balls with the intake on,
  D. exact-FRC auto-aim scoring through the physical blue HUB,
  ...
  G. FSM-driven continuous fire over N magazine runs via the shared
     RobotController.update path (FRC_CONTINUOUS_RUNS=20 -> full 160/160),
...
def main() -> None:
    ...
    try:
        ...
        from frc_rebuilt.isaac_scene import HubRouter, SceneBuilder
        from frc_rebuilt.robot_model import RobotController, quat_wxyz_to_matrix
        ...
        results["B_yaw_rate"] = {
            ...
            "frc_target_deg_per_s": 100.0,
            # +turn must be CCW (+yaw) and near the FRC-calibrated magnitude
        ...
        # FRC's coupled 6..11 m/s / 70.355..50.355 degree mechanism, and the
        ...
        for attempt in range(attempts):
            # Alliance-wall side of the blue HUB: the FRC funnel accepts this
        ...
        # for a quick gate; set FRC_CONTINUOUS_RUNS=20 for the full 160/160.
        runs_target = int(os.environ.get("FRC_CONTINUOUS_RUNS", "3"))
        teleport(1.52, -5.55, 90.0)
