...
Reads ``assets/fresh_frc/robot/{hierarchy,components,mesh_catalog}.json`` and
writes ``assets/fresh_frc/robot/robot_spec.json`` containing everything the
...
ROBOT_DIR = PROJECT / "assets" / "fresh_frc" / "robot"
...
def main() -> None:
    ...
    spec: dict = {
        "source": "assets/fresh_frc/robot (extract_robot_spec.py)",
        "frame": {
    ...
    # verified below against the L/R suffix in the FRC wheel names.
    for wheel in wheels:
