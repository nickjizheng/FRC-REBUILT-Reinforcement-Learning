#!/usr/bin/env bash
# Retry-until-boot GUI launcher for a Stage C v2 policy (prefix + suffix).
# Isaac's RTX renderer cold-render access-violates intermittently on the 5080
# laptop (Blackwell); each boot is a dice roll, so retry until one gets past
# SimulationApp render-init to PLAY_APP_READY. On success the GUI is kept alive
# (we wait on it) so this task stays up while the window is open.
set -u
PY="/c/il/venv/Scripts/python.exe"
ROOT=$(cd "$(dirname "$0")/../.." && pwd)
CKPT="runs/stagec_v2_820000.pt"
PREFIX="runs/preserved/stageC_highest_1163753.pt"
TEMPLATE="assets/rl/env_template_96.usd"
LOG="runs/v2_gui_live.log"
MAX_TRIES=8
...
for try in $(seq 1 $MAX_TRIES); do
  ...
  # if still alive at 90s without the marker, treat as slow boot and keep it
  if kill -0 "$PID" 2>/dev/null; then
