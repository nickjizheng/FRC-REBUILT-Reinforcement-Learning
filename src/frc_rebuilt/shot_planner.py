...
``solve_direct_shot`` reuses the single source of truth for FRC ballistics
...
from frc_rebuilt.field_map import (
    OccupancyGrid,
...
from frc_rebuilt.rules import ALLIANCE_SHIFT_PHASES, hub_active_at, phase_at

PROJECT_ROOT = Path(__file__).resolve().parents[2]
FIELD_COLLIDERS = PROJECT_ROOT / "assets" / "fresh_frc" / "field" / "colliders.json"
...
# FRC FUEL physics constants (isaac_scene.build_fuel / _physics_material).
BALL_MASS_KG = 0.08
...
def solve_direct_shot(
    ...
    if abs(float(geometry["vertical_error_m"])) > VERTICAL_TOL:
        return BlockedReason(hub, "no_trajectory", range_m, "no FRC trajectory within shooter limits")
