"""Build and run the high-fidelity FRC REBUILT Isaac scene."""
...
FIELD_DIR = PROJECT_ROOT / "assets" / "fresh_frc" / "field"
ROBOT_DIR = PROJECT_ROOT / "assets" / "fresh_frc" / "robot"
...
class SceneBuilder:
    ...
    def update_hub_lights(
        ...
        from frc_rebuilt.rules import HubLightState, hub_light_state_at
        ...
        for alliance in ("red", "blue"):
            ...
            base_color = FRC_HUB_LIGHT_COLORS[alliance]
            if state is HubLightState.POST_MATCH_WHITE:
                for shader in shaders:
                    self._set_hub_light_shader(
                        shader, FRC_HUB_LIGHT_COLORS["white"], 1.0
                    )
            elif state is HubLightState.WARNING:
                # Smooth FRC-style pulse; never fully black, so the alliance
    ...
    def build_robot(self) -> None:
        if self.articulated_robot:
            from frc_rebuilt.competition_robot import CompetitionRobotArticulationBuilder
...
def main() -> None:
    ...
    if not args.headless:
        try:
            ...
            print("FRC_REBUILT_FABRIC on", flush=True)
    ...
    try:
        ...
        print("FRC_REBUILT_BUILD_START", flush=True)
        context.new_stage()
        ...
        if not args.headless:
            try:
                ...
                print("FRC_REBUILT_VIEWPORT_SELECTION disabled", flush=True)
            except Exception as selection_error:
                print(
                    f"FRC_REBUILT_VIEWPORT_SELECTION_FAILED {selection_error!r}",
                    flush=True,
                )
        ...
        print("FRC_REBUILT_SCENE", json.dumps(stats), flush=True)
            ...
            with status_window.frame:
                with ui.VStack(spacing=7):
                    ...
                    status_labels["fuel_inventory"] = ui.Label(
                        f"FRC FUEL total {stats.get('fuel_total_frc', 0)}  |  "
        ...
        from frc_rebuilt.shot_planner import FieldState, MatchState, RobotState, select_target_hub
        ...
        while app.is_running() and (args.frames <= 0 or frame < args.frames):
            ...
            if frame % 2 == 0:
                ...
                # Dashboard mirrors the physical FRC light meshes, including
        ...
        print("FRC_REBUILT_PHYSICS", json.dumps(diagnostics), flush=True)
        sim.stop()
    except BaseException as error:
        import traceback
        print("FRC_REBUILT_ERROR", repr(error), flush=True)
        traceback.print_exc()
        raise
    finally:
        app.close()
