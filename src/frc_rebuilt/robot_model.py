...
Builds the robot from ``assets/fresh_frc/robot/robot_spec.json`` (produced by
``tools/extract_robot_spec.py``) as a PhysX articulation:

- chassis root link carrying the full FRC visual mesh and all solid colliders
  except the six drive-wheel spheres,
- six wheel links (sphere colliders, r=0.0808 m) on velocity-driven revolute
  joints, torque-limited so full speed matches the FRC-measured ~3.5 m/s,
- functional ball systems mirroring FRC's own trigger-volume design
...
FRC drives its robots with per-wheel torque + scripted friction forces; the
...
SPEC_PATH = PROJECT_ROOT / "assets" / "fresh_frc" / "robot" / "robot_spec.json"

# FRC-measured full-stick kinematics (see memory/docs: forward ~3.5 m/s).
...
# Differential scale so full turn stick yields FRC's ~100 deg/s yaw.
...
# Exact values decoded from the pristine FRC v20.2b Robot_RapidUS prefab.
...
# Upper FRC MarkBall trigger centers.  A physical shot is aimed through this
...
# Phase C2: solid FRC colliders that carry no enabled MeshRenderer.  These are
...
def shooter_direction_local(aim: float) -> np.ndarray:
    """Exact dynamic -local-X push axis of FRC's BS Outtake (3)."""
    angle = math.radians(SHOOTER_TRAVEL_DEG * float(np.clip(aim, 0.0, 1.0)))
    c, s = math.cos(angle), math.sin(angle)
    pitch_down = np.array([[c, 0.0, s], [0.0, 1.0, 0.0], [-s, 0.0, c]], dtype=np.float32)
...
def solve_shot_geometry(
    ...
    """Exact FRC coupled-law ballistic sweep to a HUB MarkBall opening.
    ...
    ``shot_planner.solve_direct_shot``.  Sweeps the one FRC aim parameter,
...
class RobotArticulationBuilder:
    ...
    def _visual_and_collider_geometry(self):
        from frc_rebuilt import isaac_scene as scene_mod
...
def sb_palette_robot() -> tuple[float, float, float, float]:
    from frc_rebuilt.isaac_scene import PALETTE
...
class ShooterStateMachine:
    ...
    The three FRC control modes share one latch set:
    ...
    * hold-to-fire      -> ``press_hold`` / ``release_hold`` (Button Y); release
    ...
    def press_hold(self) -> None:
        """Hold-to-fire down (FRC Button Y press)."""
        self._hold = True
        self.auto_align = True
...
class RobotController:
    ...
    ``RigidPrim`` view.  All ball handling mirrors FRC's trigger-volume
    ...
    def __init__(self, spec: dict[str, Any] | None = None):
        ...
        # (FRC renderer-disabled keep-clear + muzzle trigger volumes).
    ...
    def initialize(self, articulation: Any) -> None:
        ...
        # Robot frame: +x forward, +y left, +z up.  FRC's "L"-named wheels sit
    ...
    def preload(self, fuel_view: Any, count: int = MAGAZINE_PRELOADS) -> None:
        """Place the first ``count`` FUEL bodies in visible FRC preload slots."""
    ...
    def solve_auto_aim(self, alliance: str | None = None) -> dict[str, float | str | bool]:
        """Solve the one FRC shooter parameter and chassis heading to a HUB.
        ...
        if not on_open_side:
            ...
            best["blocked_reason"] = "no FRC trajectory within shooter limits"
        else:
            best["blocked_reason"] = ""
        best["valid"] = bool(
    ...
    def fire_auto(self, fuel_view: Any, now_s: float, alliance: str | None = None) -> int | None:
        """Fire only when the physical FRC solution is valid and aligned."""
        solution = self.solve_auto_aim(alliance)
    ...
    def follow(self, path: list[tuple[float, float]], arrival_yaw: float,
        ...
        """
        from frc_rebuilt.shot_planner import ARRIVE_TOL_M, ARRIVE_YAW_TOL_DEG, pursuit_command
